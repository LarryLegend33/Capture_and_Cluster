...
def contourparser(contours):
 if contours:
  ...
  (cnt3x,cnt3y), r = cv2.minEnclosingCircle(contour3)     
  ...
  if mindist == 0 and 15 < dist12 < 60 and 15 < dist13 < 120: 
      #typical distance is 30, 80 for inter eye and sb to either eye. Will get smaller if fish is significantly pitched, but relationship should stay the same. 
    eye1 = [contour1,int(cnt1x),int(cnt1y)]
    eye2 = [contour2,int(cnt2x),int(cnt2y)]
    swimb = [contour3,int(cnt3x),int(cnt3y)]
  elif mindist == 1 and 15 < dist13 < 60 and 15 < dist12 < 120: 
    eye1 =  [contour1,int(cnt1x),int(cnt1y)]
    eye2 = [contour3,int(cnt3x),int(cnt3y)]
    swimb = [contour2,int(cnt2x),int(cnt2y)]
  elif mindist == 2 and 15< dist23 < 60 and 15< dist13 < 120:
    eye1 =  [contour2,int(cnt2x),int(cnt2y)]
    eye2 = [contour3,int(cnt3x),int(cnt3y)]
    swimb = [contour1,int(cnt1x),int(cnt1y)]
  else:
    eye1 = []
    eye2 = []
    swimb = []
...
def eyeanglefinder(eye1,eye2,ha,center): 
  ...
  if 180 < lefteye_angle < 270:  #these calculations account for rotatedrect calling angles two ways
...
#This function just sets a lower bound on small ROIs that prevents the ROI from crossing into pixel space unoccupied by the image (i.e. no negative X values). 
...
def rotate_contour(contour, angle, center):
   ...
   for i in range(contour.shape[0]):
     ...
     xcoord_rotated = (xcoord*np.cos(angle) - ycoord*np.sin(angle)) + center[0]
     ycoord_rotated = (xcoord*np.sin(angle) + ycoord*np.cos(angle)) + center[1]
     rotated_contour.append([int(xcoord_rotated),int(ycoord_rotated)])
   rotated_cont = np.array(rotated_contour)
   return rotated_cont


#This function takes the full tail image, rotates it, makes an 500 by 500 ROI centered on the mask. finds tail by fitting a threshold sized contour, then going out along the edge of the contour by a specified number of points. at each equally spaced point, fit a point to the middle of the tail and take its angle with respect to the body axis.  writes to a movie. There are important catches for assuring that the tail is close enough to the swimb. 
...
def tailfinder(eyemidpoint,swimbcoords, tailimage, heading_angle):  
   ...
   foundtail = False
   mask_center = np.array([(eyemidpoint[0] + swimbcoords[0])/2, (eyemidpoint[1] + swimbcoords[1])/2]) #calculates point halfway between swimb and eye midpoint 
   circle_mask = np.ones(tailimage.shape,np.uint8) 
   cv2.circle(circle_mask, (int(mask_center[0]), int(mask_center[1])), 60, (0,0,0), -1) 
   #nohead will be image with a mask over the head. 
   nohead = cv2.multiply(tailimage, circle_mask).astype(np.uint8)
   nohead = cv2.cvtColor(nohead,cv2.cv.CV_BGR2GRAY)     
   lb_x,lb_y = [lower_bound(mask_center[0],win,tailimage.shape[1]),lower_bound(mask_center[1],win,tailimage.shape[0])]  
   ROItail = nohead[lb_y:lb_y+win, lb_x:lb_x+win]
   imgrot,m,c = rotate_image(ROItail,ha_adj)
   rot_color = cv2.cvtColor(imgrot,cv2.cv.CV_GRAY2RGB)  
   center = [ROItail.shape[1]/2,ROItail.shape[0]/2]
   tail_conts, hier = cv2.findContours(ROItail, cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
   ...
   tail_list = [j for j in tail_byarea if cv2.contourArea(j) > 3000 and cv2.contourArea(j) < 12000] 
   if not tail_list:   
     return False, [], rot_color
    
 #this function filters out large contours that aren't w/in 20 pixels of the swimb
   for t in tail_list: 
     dist_to_sb = [np.array(pts[0]) - swimbcoords for pts in t] 
     dist_mags = np.array([np.sqrt(np.square(x)+np.square(y)) for x,y in dist_to_sb])
     closest_pt = np.argmin(dist_mags)
     if np.min(dist_mags) < 60: #hunt through until you find the contour within 20 pixels of the swimb (see dist to swimb graph). 
       tail = t
       foundtail = True
       break
   if not foundtail:
   ...
   if closest_pt != 0: #if it is equal to zero, closest point is first element to begin with
     tail_rot =  np.concatenate([tail_rot[closest_pt:],tail_rot[0:closest_pt]])  #this just puts the closest point to the swimb as the first point in the list
   tail_rot = tail_rot[0::3].astype(np.int) #decimates the array by taking 1 point in every 3. 
   tail_perimeter = cv2.arcLength(tail_rot,True)
   tail_segment1 = []
   tail_segment2 = []
   segment = 1.0
   numsegs = 18.0
#now go out along tail until you reach 1/18 (i.e. 1/numsegs) of the total perimeter recursively. 
   for i in range(len(tail_rot)): 
     if cv2.arcLength(tail_rot[0:i+1], False) > tail_perimeter*(segment/numsegs): #False arg in arclength specifies that the contour isn't closed. 
        ...
        segment += 1
   avgtailpoints = [[np.mean([a[0],b[0]]),np.mean([a[1],b[1]])] for a,b in zip(tail_segment1,tail_segment2[::-1])]
   for tp in avgtailpoints:
     cv2.ellipse(rot_color, (int(tp[0]),int(tp[1])), (6,6), 0,0,360, (255,0,255), -1)
   ...
   #don't count first one b/c positioning of closest point is unreliable and not dependent on halving the tail width. 
...
def pitchfinder_rect(img,zcoord,x2coord, phi, fishlen,fish):
  im = cv2.cvtColor(img,cv2.cv.CV_GRAY2RGB)
  rect = cv2.minAreaRect(fish)
  box = cv2.cv.BoxPoints(rect)
  box = np.int0(box)
  x2center,zcenter = np.mean(box,axis = 0) #gives center of bounding rectangle. 
  cv2.drawContours(im,[box],0,(0,0,255),2)
  cv2.ellipse(im, (int(x2center),int(zcenter)), (2,2), 0,0,360, (0,0,255), -1) 
  cv2.ellipse(im, (int(x2coord),int(zcoord)), (2,2),0,0,360,(0,255,0), -1)
  point1 = box[0]
  distmat_wrt1 = [np.sqrt(np.dot(a-point1,a-point1)) for a in box[1:]] 
  
    #this list shows that distances from pt 1 are always mid,largest,shortest.
  if len(np.unique(distmat_wrt1)) != 3: #if it is a square, pitch is zero. have to catch this b/c can't have two identical distances for rest of calc. 
...
#angle of the long axis of the bounding rectangle
  vert_angle = np.mod(np.degrees(np.arctan2((point2[1]-point1[1]), (point2[0]-point1[0]))),360) 

  #Sometimes you get the top point and sometimes you get the bottom, which should be distinguished by correction below. 
  vert_angle_rot = np.mod(180+vert_angle,360)
  validate = [x2coord-x2center,zcoord-zcenter] #this is for validating the angle call. 
  val_angle = np.mod(np.degrees(np.arctan2(validate[1],validate[0])),360) #this is CORRECT. provides the correct angle for all frames between blue and red dot.
  angle_distances = [np.abs(180-np.abs(val_angle-vert_angle)),np.abs(180-np.abs(val_angle-vert_angle_rot))] #asks which angle is closer to 180 different
...
#Gamma is the cone derived true pitch of the fish. 
  if np.argmin(angle_distances) == 0:
   ...
   gamma = vert_angle
  #now put final vert angle from a clockwise 0-360 scale to a unit circle scale with positive and negative values. 
  if gamma <= 90:
    gamma = -1*gamma       
  elif 90 < gamma <= 180:
    gamma = -1*(180-gamma)
...
#transform this back into a z angle from the x axis. eq. of cone is 2arctan(r/h) where r is radius of cone and h is height. each gamma and phi uniquely determine a theta of a cone
  ...
  phi = np.radians(phi)
  r_over_h = 1/(np.cos(phi)*np.tan(conegamma)) #see little red notebook for this calculation
...
def get_fish_data(data_directory):
  ...
  low_res = False
  missed_fish = False
  fc = cv2.cv.CV_CAP_PROP_FRAME_COUNT
  curr_frame = cv2.cv.CV_CAP_PROP_POS_FRAMES 
  fourcc = cv2.cv.CV_FOURCC('M','J','P','G')
  cam0id = [file_id for file_id in os.listdir('D:/' + data_directory) if file_id[-8:] == 'cam0.AVI'][0]
  cam1id = [file_id for file_id in os.listdir('E:/' + data_directory) if file_id[-8:] == 'cam1.AVI'][0]
  top = cv2.VideoCapture('D:/' + data_directory + cam0id)
  side = cv2.VideoCapture('E:/' + data_directory + cam1id)
...
#  endframe = 100
  ...
  tailvid.set(curr_frame, startframe) #this won't work with startframe not zero because they only contain IR frames. 
...
#Now simply run through the videos and apply functions above to find all Varbs member lists for entire experiment. 
  ...
  for i in range(startframe,endframe,1):
    ...
    if i == 0 or i in mode_index:
      ...
      brmean_t = np.mean(ir_t_br) #normalize each frame to the mean of background 
      brmean_s = np.mean(ir_s_br)
      brcounter += 1 

    if i in epoch_boundaries:
      if ir_freq_by_epoch[0] < 10: #if freq < 10. 
          low_res = True
      else:
          low_res = False
      ir_freq_by_epoch = ir_freq_by_epoch[1:]          
    ...
    if frame_types[i] == 1:
      top.read()
      side.read() 
      #dont read tail or pitchvids b/c they only contain IR frames already. this just passes through the cam0 and cam1 frames that are fluorescent.  
      ...
      if not r:
          break
      r, pitchframe = cv2.threshold(pitchframe,120,255,cv2.THRESH_BINARY)
      r,tail = cv2.threshold(tail,120,255,cv2.THRESH_BINARY)
      contours_t, hierarchy = cv2.findContours(cv2.cvtColor(tail,cv2.cv.CV_BGR2GRAY),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
      contours_s, hierarchy = cv2.findContours(cv2.cvtColor(pitchframe,cv2.cv.CV_BGR2GRAY),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    #  varbs.nanify(['low_res_x','low_res_y','low_res_z'])

      if not r: #finds end of movies. 
          break
      ...
      if not math.isnan(fishcenter[0]):
         ...
         eye1,eye2,swimb = contourparser(conts) 
...
#this catches if contours are not found. adds nans to all member lists that are dependent on contour finding. goes to find next frame. 
      ...
      ROIcolor = cv2.cvtColor(ROI,cv2.cv.CV_GRAY2RGB)
#each return from contourparser is a list. first element is the contour itself, second is the xcoord, third is ycoord
      ...
      heading_vector = eye_midpoint - np.array([swimb[1],swimb[2]])        
      heading_angle = 360 - np.mod(np.degrees(np.arctan2(heading_vector[1], heading_vector[0])),360) 
      ...
      foundtail, tailangles, tailimage = tailfinder(eye_midpoint, np.array([swimb[1],swimb[2]]),tailROI,heading_angle)
      tailcontvid.write(tailimage) 
      leftxy,phileft,rightxy,phiright,l_or_r = eyeanglefinder(eye1,eye2,heading_angle, np.array(ROI.shape)/2)
#After operating on the ROI, return all found values to their rightful full frame coordinates. 
      ...
      swimb_fullframe = [swimb[1]+x_lb,swimb[2]+y_lb]
...
#        cv2.drawContours(ROIcolor, [eye2[0]], 0,(255,0,0), 1)
#        cv2.drawContours(ROIcolor, [eye1[0]], 0,(0,255,0), 1)
#      cv2.drawContours(ROIcolor, [swimb[0]],0, (0,0,255),1)
      contvid.write(ROIcolor) #This writes the drawn contour frames into a movie called contvid. 
      if foundtail: 
        varbs.tailangle.append(tailangles)
      else: 
        varbs.nanify(['tailangle'])
    
     # sideim[0:30,:] = correctside
      ...
      pitchframe = cv2.cvtColor(pitchframe,cv2.cv.CV_BGR2GRAY)
      ROIc, pitch, fishlength = pitchfinder_rect(pitchframe,fc_side[1],fc_side[0],heading_angle,avg_fishlength,contours_s[0]) 
      ...
      if not math.isnan(fishlength) and len(fishlength_list) < 1000:
         ...
         avg_fishlength = np.mean(fishlength_list[-20:])  #avg of the top 20 fish lengths. used to find pitch if fish is looking directy at or away from side camera. 
