...
from phinalIR import wrap_ir
from phinalFL import wrap_fluor
import os
...
def get_frametypes(dict_file):
   ...
   def assign_frames(fluor,duration,ir_freq,fl_interval):
    #provide fl_interval in seconds (ie one frame ever 10 sec), ir_freq for acquisition in hz. duration of epoch in minutes. for each, the total number of frames will be duration*60*ir_freq.
     temp_framelist = []
   ...
   frametypes = [] #0 for ir, 1 for fl
   frames_in_epoch = []
   ir_freq_in_epoch = []
   exp_dict = eval(dict_file.readline())
   for epoch,entry in enumerate(exp_dict):
          types,total_frames,ir_freq = assign_frames(*exp_dict['epoch'+ str(epoch+1)])
          frametypes += types
...
#take one frame every two seconds for the mode. modes should be calculated every 30 seconds. 
    ...
    for epoch_id, framecount in enumerate(cumulative_counts):
      print framecount
      if epoch_id == 0:
         temp_frames_br = range(0, framecount, int(frequencies[epoch_id])*br_interval)
#every 10 for temp_frames, every 150 for mode if 5 hz. every 1860 , 124 for high freq.  
         mode_indices =  range(0, framecount, int(frequencies[epoch_id])*mode_interval)
      else:
         temp_frames_br = range(cumulative_counts[epoch_id-1], framecount, int(frequencies[epoch_id])*br_interval)
         mode_indices =  range(cumulative_counts[epoch_id-1], framecount, int(frequencies[epoch_id])*mode_interval)

      frames_for_br += temp_frames_br
      mode_frames += mode_indices
    mode_frames = mode_frames[1:] # modeframes shouldn't have 0 as an index. 
...
def frametime(img):
  bytes = img[0][0:4].tolist() # timestamp info is in first row, first 4 columns.
  bits_sep = [bin(x)[2:].zfill(8) for x in bytes] #binary pattern for each integer in bytes occurs after prefix '0b' 
  bits = ''.join(bits_sep) #this joins all the binary bits into a 32 bit word.
  seconds = int(bits[0:7],2) #int function with ,2 converts a bit string back into a base 10 integer. 
  cycle = int(bits[7:20],2) 
  offset = int(bits[20:28]+'0000',2) #last 4 bits are supposedly inaccurate according to docs. replace with zeros 
  t = seconds*1000.0 + cycle*.125 + ((offset/3072.0)*.125) #returns time in ms
  return t
...
def calc_mode(deq, nump_arr):
  for j,k in enumerate(nump_arr[:,0]): #so k are the values, j are the indicies. 
    nump_arr[j,:] = mode(np.array([x[j,:] for x in deq]))[0]
    
    #pass this np.ones*255 and see if the pixels come out white
  return nump_arr 
...
def edgefix(image,x,y, isY):
  wndw = 15
  rowreplace = np.zeros(x[1]-x[0]) #has to be bounds of colindex
  colreplace = np.zeros(y[1]-y[0]) #has to be bounds of rowindex
...
data_directory = raw_input('Please Enter Experiment Directory:  ')
...
#fourcc2 = 0
ir_t_background = np.zeros([1888,1888])
ir_s_background = np.zeros([1888,1888])
top_contrasted = cv2.VideoWriter(data_directory + 'top_contrasted.AVI', fourcc2, 62, (1888,1888), False)
side_contrasted = cv2.VideoWriter(data_directory + 'side_contrasted.AVI', fourcc2,62, (1888,1888), False)
top_fl = cv2.VideoWriter(data_directory + 'top_fl.AVI', 0, 5, (1888,1888), True)
side_fl = cv2.VideoWriter(data_directory + 'side_fl.AVI', 0, 5, (1888,1888), True)
frametimes_all = []
...
if createbackground:
 ...
 for i in ir_br_frames_to_take: #make sure mode is always a multiple of frames to take. 
...
#take a mode if i is in the mode_index list (i.e. times where you should take a mode)
  ...
  if i in mode_index:   
     top_ir_backgrounds.append(calc_median(top_ir_temp, np.zeros([1888,1888])))
     side_ir_backgrounds.append(calc_median(side_ir_temp, np.zeros([1888,1888])))      
     print('after modes') 
     print(i)
     top_ir_temp = []
     side_ir_temp = []
...
for j in range(startframe,endframe,1):
 ...
 if frame_types[j] == 0:
   ...
   img_adj = (img * (brmean_top/top_avg)).astype(np.uint8)       #normalize means of br and image
   ...
   img_contrasted = imcont(top_brsub,contrast_params_top[0],contrast_params_top[1])
   img2_contrasted = imcont(side_brsub,contrast_params_side[0],contrast_params_side[1]) 
...
times_ir = [fr_time for fr_time, fr_type in zip(times_all,frame_types) if fr_type == 0]
times_fl = [fr_time for fr_time, fr_type in zip(times_all,frame_types) if fr_type == 1]
