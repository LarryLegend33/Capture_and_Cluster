...
def tailfinder(eyemidpoint,swimbcoords, tailimage, heading_angle):  
   ...
   rim, tail_conts, hier = cv2.findContours(ROItail, cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
...
def get_fish_data(data_directory):
  ...
  cam0id = [file_id for file_id in os.listdir(data_directory) if file_id[-8:] == 'cam0.AVI'][0]
  cam1id = [file_id for file_id in os.listdir(data_directory) if file_id[-8:] == 'cam1.AVI'][0]
...
#Now simply run through the videos and apply functions above to find all Varbs member lists for entire experiment. 
  ...
  for i in range(startframe,endframe,1):
    ...
    if frame_types[i] == 1:
      ...
      rim, contours_t, hierarchy = cv2.findContours(cv2.cvtColor(tail,cv2.COLOR_BGR2GRAY),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
      rim, contours_s, hierarchy = cv2.findContours(cv2.cvtColor(pitchframe,cv2.COLOR_BGR2GRAY),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
...
#can choose to base heading off position of the sb plus eyes or eyes only. 
      heading_vector_sb = eye_midpoint - np.array([swimb[1],swimb[2]])        
